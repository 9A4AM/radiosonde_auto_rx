...
class SondehubUploader(object):
    ...
    def add(self, telemetry):
        ...
        if self.input_processing_running and _telem:
		    # Add for low limit send telemetry to SondeHub by 9A4AM
    ...
    def reformat_data(self, telemetry):
        ...
        elif telemetry["type"].startswith("DFM") or (telemetry["id"].startswith("DFM")):
    ...
    def upload_telemetry(self, telem_list):
        ...
        if self.inhibit:
            self.log_info("Upload Inhibited to SondeHub -> Altitude lower then 1500m")
            return True
